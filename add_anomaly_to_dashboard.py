"""
Script để thêm phần Anomaly Detection vào dashboard
"""
...
print("="*80)
print("THÊM ANOMALY DETECTION VÀO DASHBOARD")
print("="*80)
...
# Check if anomaly section already exists
if 'ANOMALY DETECTION DATA LOADING' in dashboard_content:
    print("⚠️  Anomaly Detection section đã tồn tại trong dashboard")
    print("   Bỏ qua việc thêm mới")
else:
    # Find the position to insert (before the main function or at the end)
    if 'def main():' in dashboard_content:
        insert_pos = dashboard_content.find('def main():')
        new_content = dashboard_content[:insert_pos] + anomaly_section + '\n\n' + dashboard_content[insert_pos:]
    else:
    ...
    print("✅ Đã thêm Anomaly Detection section vào dashboard")

# Now add the tab to the main tabs
print("\n📝 Cần thêm tab 'Anomaly Detection' vào phần tabs của dashboard")
print("   Tìm dòng tạo tabs và thêm:")
print('   tab_anomaly = st.tabs(["...", "Anomaly Detection", "..."])')
print("   Sau đó thêm:")
print('   with tab_anomaly:')
print('       render_anomaly_detection_tab()')
